...
Intution : https://www.youtube.com/watch?v=4uUGxZXoR5o
**/
class Solution {
public:
  int calculateMinimumHP(vector<vector<int>>& d) {
    ...
    for(int i=m-1; i>=0; i--) {
      for(int j=n-1; j>=0; j--) {
        if(i==m-1 && j==n-1) t[i][j] = min(0, d[i][j]);
        else if(i==m-1) t[i][j] = min(0, t[i][j+1] + d[i][j]);
        else if(j==n-1) t[i][j] = min(0, t[i+1][j] + d[i][j]);
        else t[i][j] = min(0, max(t[i][j+1],t[i+1][j])+d[i][j]);
      }
    }
    return abs(t[0][0])+1;
  }
