...
Starting from 1 , land on square 100 with the exact roll of the die.
...
Input Format
First argument is a 2D matrix A of size N * 2 where (A[i][0], A[i][1])
denotes a ladder that has its base on square A[i][0] and end at square A[i][1].
Second argument is a 2D matrix B of size M * 2 where (B[i][0], B[i][1])
denotes a snake that has its mouth on square B[i][0] and tail at square B[i][1].
Output Format
...
int Solution::snakeLadder(vector<vector<int> > &A, vector<vector<int> > &B) {
  ...
  for(int i=0; i<A.size(); i++) mA[A[i][0]] = A[i][1];
  for(int i=0; i<B.size(); i++) mB[B[i][0]] = B[i][1];
  ...
  while(!q.empty()) {
    ...
    for(int i=1; i<=6; i++) {
      int newPos = curPos.first + i;
      int dist = curPos.second + 1;
      if(newPos > 100)
        break;
      if(visited[newPos])
        continue;
      if(mA.find(newPos) != mB.end())
        newPos = mA[newPos];
      else if(mB.find(newPos) != mB.end())
        newPos = mB[newPos];
      q.push({newPos, dist});
      visited[newPos] = true;
    }
