...
class Solution {
public:
  void dfs(TreeNode *A, map<int,multiset<pair<int,int>>> &m, int idx, int row) {
    if(!A) return;
    m[idx].insert({row,A->val});
    dfs(A->left, m, idx-1, row+1);
    dfs(A->right, m, idx+1, row+1);
  }
